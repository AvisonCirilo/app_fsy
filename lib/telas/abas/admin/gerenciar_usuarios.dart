...
import 'package:firebase_core/firebase_core.dart'; // Necessário para a mágica de não deslogar o admin!
...
class _GerenciarUsuariosTelaState extends State<GerenciarUsuarios> {
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        if (!isEdicao) // Só mostra o campo de senha se for criação
                          Expanded(
                        ...
                        onPressed: () async {
                          ...
                          try {
                            if (isEdicao) {
                              ...
                              // MÁGICA: Cria uma instância secundária para não deslogar o Admin!
                              ...
                              await FirebaseFirestore.instance.collection('usuarios').doc(cred.user!.uid).set({
                                ...
                                'saude': saudeCtrl.text,
                                'medicamento': medicamentoCtrl.text,
                                'companhia': 'A Definir', // Padrão
                                'criadoEm': FieldValue.serverTimestamp(),
                              });
                              
                              await appSecundario.delete(); // Deleta a instância invisível
                            }
                            // ignore: use_build_context_synchronously
                            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Utilizador ${isEdicao ? 'atualizado' : 'criado'} com sucesso!"), backgroundColor: Colors.green));
                          } catch (e) {
                            // ignore: use_build_context_synchronously
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // MÁGICA 2: O STREAMBUILDER! Lê os dados da nuvem em tempo real!
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return Center(child: Text("Nenhum utilizador encontrado.", style: TextStyle(color: Colors.grey.shade600)));
          }
                    ...
                    return await showDialog(
                      ...
                      builder: (BuildContext context) {
                        return AlertDialog(
                          ...
                          title: const Row(
                            children: [Icon(Icons.warning_amber_rounded, color: Colors.redAccent, size: 28), SizedBox(width: 10), Text("Excluir Utilizador")],
                          ),
                          content: Text("Tem a certeza que deseja excluir ${usuario['nome']} do sistema?\n\nEle perderá o acesso ao app.", style: const TextStyle(fontSize: 15)),
                          actions: [
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () async {
                                ...
                                // Exclui a ficha no banco de dados!
